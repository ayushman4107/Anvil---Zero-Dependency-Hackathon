...
|---|---|---|---|
| Implemented through Phase 8; recorded in `STDLIB.md` | `github.com/valyala/fasthttp` or a framework HTTP engine | `net`, `bufio`, `io`, `bytes` plus Anvil's HTTP/1.1 codec | Implements TCP lifecycle, message framing, keep-alive, chunked coding, strict errors, and hardening limits directly |
...
| Implemented in Phase 6; recorded in `STDLIB.md` | `github.com/prometheus/client_golang` | `sync/atomic`, fixed histogram buckets, `runtime/metrics`, custom JSON output | Bounded metrics and estimated latency percentiles without a collector library |
| Implemented in Phase 6; recorded in `STDLIB.md` | `github.com/r3labs/sse` or a WebSocket telemetry package | Custom chunked HTTP writer, `bufio`, bounded channels | Standards-compatible one-way live dashboard updates with replay and drops |
...
| Implemented through Phase 8; recorded in `STDLIB.md` | `github.com/spf13/cobra` | `flag`, `os.Args` | Explicit subcommands, bounded experiment/benchmark options, help, and exit behavior |
| Partially implemented through Phase 8; recorded in `STDLIB.md` | `github.com/spf13/viper` | `encoding/json`, `os`, `flag` | Strict size-limited experiment JSON ships; general proxy route/pool JSON remains mandatory |
...
| Implemented through Phase 8; recorded in `STDLIB.md` | `github.com/json-iterator/go` | `encoding/json` | Events, metrics, strict bounded scenarios, and receipts ship |
...
| Implemented through Phase 8; recorded in `STDLIB.md` | `github.com/stretchr/testify` | `testing`, explicit clock seams | Table tests, deterministic state transitions/schedules, real sockets, four fuzz targets, allocation benchmarks, three-run experiments, and native race detection |
